...
public class ACRA {
    ...
    static class C00281 implements OnSharedPreferenceChangeListener {
        C00281() {
        }
    ...
    public static void init(Application app) {
        ...
        if (mReportsCrashes == null) {
            log.m4e(LOG_TAG, "ACRA#init called but no ReportsCrashes annotation on Application " + mApplication.getPackageName());
            return;
        }
        SharedPreferences prefs = getACRASharedPreferences();
        try {
            checkCrashResources();
            log.m2d(LOG_TAG, "ACRA is enabled for " + mApplication.getPackageName() + ", intializing...");
            ...
            log.m11w(LOG_TAG, "Error : ", e);
        }
        mPrefListener = new C00281();
    ...
    static void checkCrashResources() throws ACRAConfigurationException {
        ...
        switch (C00292.$SwitchMap$org$acra$ReportingInteractionMode[conf.mode().ordinal()]) {
    ...
    public static ACRAConfiguration getConfig() {
        if (configProxy == null) {
            if (mApplication == null) {
                log.m10w(LOG_TAG, "Calling ACRA.getConfig() before ACRA.init() gives you an empty configuration instance. You might prefer calling ACRA.getNewDefaultConfig(Application) to get an instance with default values taken from a @ReportsCrashes annotation.");
            }
