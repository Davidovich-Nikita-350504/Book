...
public class GoogleAnalyticsTracker {
    ...
    final class DispatcherCallbacks implements Callbacks {
        ...
        public void dispatchFinished() {
            GoogleAnalyticsTracker.this.handler.post(new C00011());
        }
    ...
    private GoogleAnalyticsTracker() {
        ...
        this.dispatchRunner = new C00001();
    }
    ...
    public void trackTransactions() {
        for (Transaction transaction : this.transactionMap.values()) {
            Event event = new Event(this.accountId, "__##GOOGLETRANSACTION##__", ACRAConstants.DEFAULT_STRING_VALUE, ACRAConstants.DEFAULT_STRING_VALUE, 0, this.parent.getResources().getDisplayMetrics().widthPixels, this.parent.getResources().getDisplayMetrics().heightPixels);
            event.setTransaction(transaction);
            this.hitStore.putEvent(event);
            Map map = (Map) this.itemMap.get(transaction.getOrderId());
            if (map != null) {
                for (Item item : map.values()) {
                    event = new Event(this.accountId, "__##GOOGLEITEM##__", ACRAConstants.DEFAULT_STRING_VALUE, ACRAConstants.DEFAULT_STRING_VALUE, 0, this.parent.getResources().getDisplayMetrics().widthPixels, this.parent.getResources().getDisplayMetrics().heightPixels);
                    event.setItem(item);
                    this.hitStore.putEvent(event);
                }
            }
